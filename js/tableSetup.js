let tableSetup = {
    ...
    rotateOrMoveShip: function (properties, targetProperties) {
        if (properties.rotateOrMove === "rotate" ){
            targetProperties.newPosition = this.createNewRotatedPosition(targetProperties)
        } else if(properties.rotateOrMove === "move") {
            targetProperties.newPosition = this.createNewMovedPosition(targetProperties, properties.newPlace);
        }
        let newTableAndShip = this.removeOldPosition(properties, targetProperties);
        properties.table = newTableAndShip.table;
        properties.shipPosition = newTableAndShip.shipPosition;
        if (this.isPositionEmpty(properties.table, targetProperties) === true) {
            let newTableAndShip = this.addTranslatedShip(properties, targetProperties);
            properties.table = newTableAndShip.table;
            properties.shipPosition = newTableAndShip.shipPosition;
        } else {
            let oldTableAndShip = this.returnOldPosition(properties, targetProperties);
            properties.table= oldTableAndShip.table;
            properties.shipPosition = oldTableAndShip.shipPosition;
        }
        return {table: properties.table, shipPosition: properties.shipPosition};
    },

    createNewMovedPosition: function (targetProperties, newPlace) {
        let newPosition = [],
    ...
    addTranslatedShip: function (properties, obj) {
        let newDirection;
        if (properties.rotateOrMove === "rotate") {
        ...
        properties.shipPosition[obj.shipNumber] = {
            position: obj.newPosition,
            direction: newDirection
        };
        for (let cell = 0; cell < obj.newPosition.length; cell++) {
            properties.table[obj.newPosition[cell][0]][obj.newPosition[cell][1]] = labels.ship;
        }
        properties.table = this.updateReservedSpace(properties.table);
        return {table: properties.table, shipPosition: properties.shipPosition};
    },

    removeOldPosition: function (properties, obj) {
        delete properties.shipPosition[obj.shipNumber];
        for (let cell = 0; cell < obj.oldPosition.length; cell++) {
            properties.table[obj.oldPosition[cell][0]][obj.oldPosition[cell][1]] = labels.emptySpace;
        }
        properties.table = this.updateReservedSpace(properties.table);
        return {table: properties.table, shipPosition:properties.shipPosition}
    },
    ...
    returnOldPosition: function (properties, obj) {
        properties.shipPosition[obj.shipNumber] = {
            position: obj.oldPosition,
            direction: obj.oldDirection
        };
        for (let cell = 0; cell < obj.oldPosition.length; cell++) {
            properties.table[obj.oldPosition[cell][0]][obj.oldPosition[cell][1]] = labels.ship;
        }
        properties.table = this.updateReservedSpace(properties.table);
        return {table:properties.table, shipPosition:properties.shipPosition};
    },
